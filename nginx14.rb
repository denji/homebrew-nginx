...
class Nginx14 < Formula
  ...
  def install
    args = ["--prefix=#{prefix}",
            ...
            "--with-pcre",
            "--with-ipv6",
            "--sbin-path=#{bin}/nginx14",
            "--with-cc-opt=-I#{HOMEBREW_PREFIX}/include",
            "--with-ld-opt=-L#{HOMEBREW_PREFIX}/lib",
            "--conf-path=#{etc}/nginx14/nginx.conf",
            "--pid-path=#{var}/run/nginx14.pid",
            "--lock-path=#{var}/run/nginx14.lock",
            "--http-client-body-temp-path=#{var}/run/nginx14/client_body_temp",
            "--http-proxy-temp-path=#{var}/run/nginx14/proxy_temp",
            "--http-fastcgi-temp-path=#{var}/run/nginx14/fastcgi_temp",
            "--http-uwsgi-temp-path=#{var}/run/nginx14/uwsgi_temp",
            "--http-scgi-temp-path=#{var}/run/nginx14/scgi_temp",
            "--http-log-path=#{var}/log/nginx14",
    ...
    # for such cases
    if (HOMEBREW_CELLAR/'nginx14').subdirs.any?{|d| (d/:sbin).directory? }
      sbin.mkpath
      sbin.cd do
        (sbin/'nginx14').make_relative_symlink(bin/'nginx14')
      end
    end
  end
  ...
  def caveats; <<-EOS.undent
    ...
    The default port has been set to 8084 so that nginx can run without sudo.

    If you want to host pages on your local machine to the wider network you
    can change the port to 80 in: #{HOMEBREW_PREFIX}/etc/nginx14/nginx.conf
  ...
  def plist; <<-EOS.undent
    ...
    <plist version="1.0">
      <dict>
        ...
        <array>
            <string>#{opt_prefix}/bin/nginx14</string>
            <string>-g</string>
